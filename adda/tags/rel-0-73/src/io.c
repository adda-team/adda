...
/*    else if (strcmp(argv[i],"-sdisk_rot")==0) {
    ...
    /*    else if (strcmp(argv[i],"-prisma")==0) {
      ...
      printz("illegal option \"%s\"\n"\
             "Usage: adda {-alldir_inp %%s|-asym|\n"\
             "-beam {plane|{buggy|barton1|barton3|barton5|davis1|davis3|lminus} %%f %%f %%f %%f}|\n"\
             "-chp_dir %%s|-chp_load|-chp_type {normal|regular|always}|-chpoint %%s|\n"\
             "-Cpr_mat|-Csca|-dir %%s|-dpl %%f|-eps %%f|-grid %%d [%%d %%d]|\n"\
             "-int {poi|so}|-iter {cgnr|bicg|bicgstab|qmr}|-jagged %%d|-lambda %%f|\n"\
             "-m %%f %%f [%%f %%f...]|-maxiter %%d|-no_reduced_fft|-no_vol_cor|\n"\
             "-nosym|-ntheta %%d|-orient {%%f %%f %%f|avg [%%s]}|\n"\
             "-phi_integr %%d|-pol {cm|rrc|ldr [avg]|cldr|so}|-prognose|-prop %%f %%f %%f|\n"\
             "-save_geom [%%s]|-scat {dr|so}|-scat_grid_inp %%s|\n"\
             "-shape {box|coated %%f [%%f %%f %%f]|cylinder %%f|ellipsoid %%f %%f|line|\n"\
             "        rbc %%f %%f %%f|read %%s|sphere|spherebox}|\n"\
             "-size %%f|-store_force|-store_int_field|-store_scat_grid|-sym_enf|\n"\
             "-test|-vec|-yz}...\n\n",argv[i]);
      Stop(1);
    }
    i++;
