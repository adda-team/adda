...
extern double *tab1,*tab2,*tab3,*tab4,*tab5,*tab6,*tab7,*tab8,*tab9,*tab10;
...
int blockTr=TR_BLOCK;        /* block size for TransposeYZ; see fft.h */
int weird_nprocs;            /* whether weird number of processors is used */
#ifdef FFTW3
...
INLINE int IndexDmatrix(int x,int y,int z)
...
INLINE int IndexGarbledD(int x,int y,int z, int lengthN)
...
INLINE int IndexD2matrix(int x,int y,int z, int nnn)
...
INLINE int IndexSlice_zyD2matrix(int y,int z)
     ...
     called in the beginning from InitComm */
{
...
#ifdef FFT_TEMPERTON
  if (y!=1) {
    fprintz(stderr,"Specified number of processors (%d) is weird\n"\
  ...
  while (y%7==0) y/=7;
  if (y!=1) {
    printz("\nWARNING: Specified number of processors (%d) is weird\n"\
           "   (has prime divisors larger than 7). FFTW3 will work less efficiently.\n"\
           "   It is strongly recommended to revise the number of processors.\n\n",nprocs);
    weird_nprocs=TRUE;
  }
#endif
}
...
void fftInitBeforeD(int lengthZ)
