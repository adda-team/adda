...
extern int store_scat_grid;
extern int calc_Cext;
extern int calc_Cabs;
extern int calc_Csca;
extern int calc_vec;
extern int calc_asym;
extern int calc_mat_force;
...
void ComputeMuellerMatrix(double matrix[4][4], doublecomplex s1, doublecomplex s2,
                          doublecomplex s3, doublecomplex s4)
...
void ComputeMuellerMatrixNorm(double matrix[4][4], doublecomplex s1, doublecomplex s2,
                              doublecomplex s3, doublecomplex s4)
  ...
  if (orient_avg) {                 /* Amplitude matrix stored in ampl_alplha is */
    ...
    if (scat_grid) {
      ...
      if (store_scat_grid) {
        ...
        mueller=fopen(stringbuffer, "w");
        if (mueller==NULL)
          LogError(EC_ERROR,ONE,POSIT,"File 'mueller_scatgrid' write error");
      ...
      if (phi_integr) {   /* also initializes arrays of multipliers */
        if (phi_int_type & PHI_UNITY) {
          ...
          if (mueller_int==NULL)
            LogError(EC_ERROR,ONE,POSIT,"File 'mueller_integr' write error");
        ...
        if (phi_int_type & PHI_COS2) {
          ...
          if (mueller_int_c2==NULL)
            LogError(EC_ERROR,ONE,POSIT,"File 'mueller_integr_c2' write error");
        ...
        if (phi_int_type & PHI_SIN2) {
          ...
          if (mueller_int_s2==NULL)
            LogError(EC_ERROR,ONE,POSIT,"File 'mueller_integr_s2' write error");
        ...
        if (phi_int_type & PHI_COS4) {
          ...
          if (mueller_int_c4==NULL)
            LogError(EC_ERROR,ONE,POSIT,"File 'mueller_integr_c4' write error");
        ...
        if (phi_int_type & PHI_SIN4) {
          ...
          if (mueller_int_s4==NULL)
            LogError(EC_ERROR,ONE,POSIT,"File 'mueller_integr_s4' write error");
      ...
      if (phi_integr) { /* also free arrays */
        if (phi_int_type & PHI_UNITY) fclose(mueller_int);
        if (phi_int_type & PHI_COS2) {
...
void CalcEplane(char which,int type)
...
void CalcIntegralScatQuantities(char which)
    ...
    Cext and Cabs are overaged over orientation, if needed. */
{
  ...
  if (orient_avg) {
    ...
    if (ringid==ROOT) {
      ...
      if (calc_Cext) {
        printf("Cext\t= %.10g\nQext\t= %.10g\n",Cext,Cext*inv_G);
        fprintf(CCfile,"Cext\t= %.10g\nQext\t= %.10g\n",Cext,Cext*inv_G);
      }
      if (calc_Cabs) {
        printf("Cabs\t= %.10g\nQabs\t= %.10g\n",Cabs,Cabs*inv_G);
        fprintf(CCfile,"Cabs\t= %.10g\nQabs\t= %.10g\n",Cabs,Cabs*inv_G);
      }
      if (all_dir) fprintf(CCfile,"\nIntegration\n\n");
      if (calc_Csca) {
        Csca=ScaCross();
        printf("Csca\t= %.10g\nQsca\t= %.10g\t  (integration)\n\n",Csca,Csca*inv_G);
        fprintf(CCfile,"Csca\t= %.10g\nQsca\t= %.10g\n",Csca,Csca*inv_G);
      }
      if (calc_vec) {
        AsymParm_x(dummy);
        printf("g.Csca-x\t= %.10g\n",dummy[0]);
        if (calc_asym) printf("g-x\t\t= %.10g\n",g[0]=dummy[0]/Csca);
        AsymParm_y(dummy+1);
        printf("g.Csca-y\t= %.10g\n",dummy[1]);
        if (calc_asym) printf("g-y\t\t= %.10g\n",g[1]=dummy[1]/Csca);
        AsymParm_z(dummy+2);
        printf("g.Csca-z\t= %.10g\n",dummy[2]);
        if (calc_asym) printf("g-z\t\t= %.10g\n",g[2]=dummy[2]/Csca);
          fprintf(CCfile,"Csca.g\t=(%.10g,%.10g,%.10g)\n",dummy[0],dummy[1],dummy[2]);
        if (calc_asym) fprintf(CCfile,"g\t=(%.10g,%.10g,%.10g)\n",g[0],g[1],g[2]);
      }
    } /* end of ROOT */
    if (calc_mat_force) {
...
void StoreIntFields(char which)
    /* Write actual internal fields (not exciting) on each dipole to file
       ...
       use -no_sym if needed */
{
  FILE *intfld_file;	   /* file to store internal fields */
  double V;
  ...
  if ((intfld_file=fopen(stringbuffer,"w"))==NULL)
    LogError(EC_ERROR,ONE,POSIT,"Could not write to file '%s'",stringbuffer);
  if (ringid==ROOT)
    fprintf(intfld_file,"x y z |E|^2 Ex.r Ex.i Ey.r Ey.i Ez.r Ez.i\n");
  /* saves fields to file */
  for (i=0;i<local_nvoid_Ndip;++i) {
    mat=material[i];
    for (j=0;j<3;j++) cMult(hi_inv[mat],pvec[3*i+j],fld[j]);   /* field=P/(V*hi) */
    fprintf(intfld_file,
...
int CalculateE(char which,int type) /* x or y polarized incident light */
{
  int exit_status;
