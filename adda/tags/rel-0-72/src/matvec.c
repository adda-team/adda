...
extern doublecomplex *Dmatrix,*Xmatrix,*slices,*slices_tr;
extern int DsizeY,DsizeZ,DsizeYZ,NDcomp;

/*============================================================*/

INLINE int IndexSliceZY(int y,int z)
{
  return (z*gridY+y);
}
...
INLINE int IndexSliceYZ(int y,int z)
{
  return(y*gridZ+z);
}
...
INLINE int IndexGarbledX(int x,int y,int z)
{
#ifdef PARALLEL
...
INLINE int IndexXmatrix(int x,int y,int z)
{
  return((z*smallY+y)*gridX+x);
}
...
INLINE int IndexDmatrix_mv(int x,int y,int z, int transposed)
{
