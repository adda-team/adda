...
doublecomplex *Eper,*Epar,*Egrid;
double *Eparper_buffer,*Egrid_buffer;
...
void save_in_mie(void)
{
  ...
  r=(2.0*PI/dpl)*pow((0.75/PI)*nvoid_Ndip,.333333333333);
  ...
  if (shape==COATED) {
    ...
    outer=0;
    rin=coat_ratio*r;
    rin=2.0*PI*pow((0.75/PI)*mat_count[1]/(dpl*dpl*dpl),.333333333333);
  }
...
void Calculator (void)
{
  ...
  if (all_dir) {
    ...
    temp_int=3*parms[THETA].Grid_size*parms[PHI].Grid_size;
    if (prognose == false) {
      if ((Egrid = dCvector(temp_int)) == NULL)
    ...
    memory+=temp_int*sizeof(doublecomplex);
#ifdef PARALLEL
