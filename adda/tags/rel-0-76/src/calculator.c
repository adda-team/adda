  ...
  /* allocates a lot of arrays and performs memory analysis */
{
  ...
  memory+=5*nlocalRows*sizeof(doublecomplex);
  if (IterMethod==IT_BICGSTAB || IterMethod==IT_QMR_CS) {
    ...
    memory+=3*nlocalRows*sizeof(doublecomplex);
  }
  if (yzplane) {
    if (!prognose) {
      MALLOC_VECTOR(EplaneX,complex,2*nTheta,ALL);
      MALLOC_VECTOR(EplaneY,complex,2*nTheta,ALL);
    }
    memory+=4*nTheta*sizeof(doublecomplex);
#ifdef PARALLEL
    if (ringid==ROOT) { /* buffer for accumulate operation */
      if (!prognose) MALLOC_VECTOR(Eplane_buffer,double,4*nTheta,ONE);
      memory+=4*nTheta*sizeof(double);
    }
#endif
  }
  if (all_dir) {
    ...
    if (!prognose) MALLOC_VECTOR(E2_alldir,double,temp_int,ALL);
  ...
  if (scat_grid) {
    ...
    if (phi_integr && ringid==ROOT) {
      if (!prognose) {
        MALLOC_VECTOR(muel_phi,double,16*angles.phi.N+2,ONE);
        MALLOC_VECTOR(muel_phi1,double,16*angles.phi.N+2,ONE);
      }
      memory+=2*16*angles.phi.N*sizeof(double);
    }
#ifdef PARALLEL
    if (ringid==ROOT) {  /* buffer for accumulate operation */
      if (!prognose) MALLOC_VECTOR(Egrid_buffer,double,2*temp_int,ONE);
      memory+=2*temp_int*sizeof(double);
    }
#endif
  }
  if (orient_avg) {
    if (!prognose) {
      MALLOC_VECTOR(ampl_alphaX,complex,2*nTheta*alpha_int.N,ONE);
      MALLOC_VECTOR(ampl_alphaY,complex,2*nTheta*alpha_int.N,ONE);
    }
    ...
    if (ringid==ROOT) {
      ...
      memory += (block_theta*(alpha_int.N+1)+4)*sizeof(double);
    }
  }
  /* estimate of the memory (only the fastest scaling part):
       MatVec - (288+384nprocs/boxX [+192/nprocs])*Ndip
                more exactly: gridX*gridY*gridZ*(72+384nprocs/boxX [+48/nprocs])
