    ...
    */
{
  ...
  if (shape==SH_BOX) {
    strcpy(sh_form_str,"cube; size of edge:%.10g");
    symX=symY=symZ=TRUE;
    ...
    sprintf(sh_form_str,"coated sphere; diameter(d):%%.10g, inner diameter d_in/d=%.10g",coat_ratio);
    if (sh_Npars==4) {
      coat_x=sh_pars[1];
      coat_y=sh_pars[2];
      coat_z=sh_pars[3];
    ...
    sprintf(sh_form_str,"cylinder; diameter(d):%%.10g, height h/d=%.10g",diskratio);
    symX=symY=symZ=symR=TRUE;
    ...
    sprintf(sh_form_str,"ellipsoid; size along X:%%.10g, aspect ratios y/x=%.10g, z/x=%.10g",
            ellipsY,ellipsZ);
    symX=symY=symZ=TRUE;
    ...
    sprintf(sh_form_str,"specified by file %s; size along X:%%.10g",aggregate_file);
    ...
    strcpy(sh_form_str,"sphere; diameter:%.10g");
    symX=symY=symZ=symR=TRUE;
    volume_ratio=PI/6;
    n_boxY=n_boxZ=boxX;
    Nmat_need=1;
...
/*        else if  (shape==SH_SDISK_ROT) {
        ...
        position_tmp[3*index]=(short int)i;
        position_tmp[3*index+1]=(short int)j;
        position_tmp[3*index+2]=(short int)k;
