...
void Accumulate(double *data,int size,double *buffer,TIME_TYPE *timing);
void MyInnerProduct(void *a,var_type type,int n_elem,TIME_TYPE *timing);
void AllGather(void *x_from,void *x_to,var_type type,int n_elem);
void InitComm(int *argc_p,char ***argv_p);
void ParSetup(void);
void MyBcast(void *data,var_type type,int n_elem,TIME_TYPE *timing);
void BcastOrient(int *i,int *j,int *k);
  /* used by granule generator */
void SetGranulComm(double z0,double z1,double gdZ,int gZ,int gXY,int buf_size,int *lz0,
                   int *lz1,int sm_gr);
void CollectDomainGranul(unsigned char *dom,int gXY,int lz0,int locgZ,TIME_TYPE *timing);
