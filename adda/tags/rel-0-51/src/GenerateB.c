...
GenerateB (char which,		/* x - or y polarized incident light */
           ...
           double **dip_coord,	/* matrix containing dipole coordinates */
...
{ 
  ...
  if (which=='Y') {p1=1-3*local_d0; p2=0-3*local_d0; p3=2-3*local_d0; incPol=incPolY;}
  if (which=='X') {p1=0-3*local_d0; p2=1-3*local_d0; p3=2-3*local_d0; incPol=incPolX;}
  if (beamtype==PLANE)
    for (i = 0; i < nldip; ++i) {
      tmpArg=k*DotProd(dip_coord[i],prop);
      ctemp.r=cos(tmpArg);
      ctemp.i=sin(tmpArg);
      cScalMultRVec(incPol,ctemp,&(b[3*i])); /* b[3*i]=ctemp*incPol */
    } 
