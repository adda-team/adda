...
double Romberg1D(/* Performs integration of data */
   ...
   /* Since all values are already calculated, no adaptation is used (all data is used)
      ...
      returns mean-square error; if function is periodic then only the first column
   ...
   /* allocates all needed memory */
{
  ...
  M_in = dMatrix(0,size_in,0,dim-1);
  T_in = (double *)malloc(dim*sizeof(double));
  dummy_in = (double *)malloc(dim*sizeof(double));
  /* outer loop vectors */
  size_out=input[THETA].periodic ? 0 : input[THETA].Jmax;
  M_out = dMatrix(0,size_out,0,dim-1);
  T_out = (double *)malloc(dim*sizeof(double));
  dummy_out = (double *) malloc(dim*sizeof(double));
  /* common to fasten calculations; needed only for really Romberg */
  maxdim=MAX(size_in,size_out);
  if (maxdim!=0) {
    tv1 = dVector(0,maxdim);
    tv2 = dVector(1,maxdim);
    tv3 = dVector(0,maxdim);
    tv1[0]=1;
    for (i=1;i<maxdim;i++) {
      tv1[i]=tv1[i-1]*4;
      tv2[i]=1/(tv1[i]-1);
...
{
  /* inner */
  Free_dMatrix(M_in,0,size_in,0);
  free(T_in);
  free(dummy_in);
  /* outer */
  Free_dMatrix(M_out,0,size_out,0);
  free(T_out);
  free(dummy_out);
  /* common */
...
static double InnerRomberg(const int fixed,double *res)
  /* Integrate (average) func for fixed theta=fixed; returns estimate of the absolute error;
     if function is periodic then only the first column of the table is used
     - i.e. trapezoid rule */
{
  int m,m0,comp;
