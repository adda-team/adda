...
extern int shape,sh_Npars;
extern double sh_pars[];
extern int NoSymmetry,symmetry_enforced;
extern double sizeX,dpl,lambda;
extern int jagged;
extern char aggregate_file[],save_geom_fname[],shapename[];
extern int volcor,save_geom;
/* defined and initialized in timing.c */
extern clock_t Timing_Particle;

/* used in io.c */
int volcor_used;         /* volume correction was actually employed */
char sh_form_str[200];   /* string for log file with shape parameters */

/* LOCAL VARIABLES */

char geom_format[] = "%d %d %d\n";         /* format of the geom file */
char geom_format_ext[] = "%d %d %d %d\n";  /* extended format of the geom file */
double volume_ratio; /* ratio of scatterer volume to enclosing cube; used for dpl correction */
int Ndip;            /* total number of dipoles (in a circumscribing cube) */
/* shape parameters */
double coat_ratio,coat_x,coat_y,coat_z,coat_r2;
double diskratio,ellipsY,ellipsZ;
double P,Q,R,S;         /* for RBC */
...
#endif
    fprintf(geom,"#generated by ADDA v.%s\n"\
                 "#shape: '%s'\n"\
                 "#box size: %dx%dx%d\n",
            version,shapename,boxX,boxY,boxZ);
     ...
     returns number of lines skipped */
{
  ...
  while (fgetc(file)=='#') {
    do fgets(buf,buf_size,file); while (strstr(buf,"\n")==NULL);
    lines++;
  }
...
void InitDipFile(char *filename, int *maxX, int *maxY, int *maxZ, int *Nm)
   /* read dipole file first to determine box sizes and Nmat */
{
  ...
  line=SkipComments(input);
  /* read Nmat if present */
  if (fscanf(input,"Nmat=%d\n",Nm)!=1) *Nm=1;
  else {
    ext=TRUE;
    if (*Nm<=0) LogError(EC_ERROR,ONE,POSIT,"Nmat is nonpositive, as given in %s",filename);
    if (*Nm==1) LogError(EC_WARN,ONE,POSIT,
                         "Nmat is given in dipole file - %s, but is trivial (=1)",filename);
    line++;
  }
  /* scan main part of the file */
  *maxX=*maxY=*maxZ=0;
  while(!feof(input)) {
    if (ext) cond=(fscanf(input,geom_format_ext,&x,&y,&z,&mat)!=4);
    else cond=(fscanf(input,geom_format,&x,&y,&z)!=3);
    if (cond)
      LogError(EC_ERROR,ONE,POSIT,"Could not scan from dipole file - %s - line %d",filename,line);
    /* check for errors in values */
    if (x<0)
      LogError(EC_ERROR,ONE,POSIT,"Negative coordinate - %s - line %d: x=%d",filename,line,x);
    if (y<0)
      LogError(EC_ERROR,ONE,POSIT,"Negative coordinate - %s - line %d: y=%d",filename,line,y);
    if (z<0)
      LogError(EC_ERROR,ONE,POSIT,"Negative coordinate - %s - line %d: z=%d",filename,line,z);
    if (ext) if (mat>*Nm) LogError(EC_ERROR,ONE,POSIT,
      "Given material number - %s - line %d: mat=%d is greater than provided Nmat (%d)",
      filename,line,mat,*Nm);
    /* update maximums */
    if (x>*maxX) *maxX=x;
    if (y>*maxY) *maxY=y;
    if (z>*maxZ) *maxZ=z;
       ...
       no consistency checks are made since they are made in InitDipFile */
{
  ...
  while(!feof(input)) {
    if (ext) fscanf(input,geom_format_ext,&x0,&y0,&z0,&mat);
    else fscanf(input,geom_format,&x0,&y0,&z0);
    ...
    */
{
  ...
  if (boxX==UNDEF) {
    if (shape!=SH_READ) {
      if (dpl!=UNDEF && sizeX!=UNDEF) {
        boxX=FitBox(ceil(sizeX*dpl/lambda));
        dpl=UNDEF;     /* dpl is given correct value below */
      }
      else boxX=16*jagged; /* default value for boxX */
    }
  }
  else {
    temp=boxX;
    if ((boxX=FitBox(boxX))!=temp)
      LogError(EC_WARN,ONE,POSIT,"boxX has been adjusted from %i to %i",temp,boxX);
  }
  n_boxX=boxX;
...
/*  else if(shape==SH_SDISK_ROT) {
  ...
  else if (n_boxX>boxX) LogError(EC_ERROR,ONE,POSIT,
  ...
  if (boxY==UNDEF) { /* assumed that boxY and boxZ are either both defined or both not defined */
    ...
    if ((boxY=FitBox(boxY))!=temp)
      LogError(EC_WARN,ONE,POSIT,"boxY has been adjusted from %i to %i",temp,boxY);
    temp=boxZ;
    if ((boxZ=FitBox(boxZ))!=temp)
      LogError(EC_WARN,ONE,POSIT,"boxZ has been adjusted from %i to %i",temp,boxZ);
    /* this error is not duplicated in the logfile since it does not yet exist */
    if (n_boxY>boxY || n_boxZ>boxZ) LogError(EC_ERROR,ONE,POSIT,
    ...
    for(j=-boxY/2;j<boxY/2;j++)
      for(i=-boxX/2;i<boxX/2;i++) {
        ...
        if (shape==SH_BOX) mat=0;
          ...
          if((abs(zr) <= diskratio/2) && (xr*xr + yr*yr <= 0.25)) mat = 0;
        }
        else if (shape==SH_ELLIPSOID) {
...
/*        else if  (shape==SH_SDISK_ROT) {
  ...
  if (sizeX==UNDEF) {
    if (dpl==UNDEF) dpl=10*sqrt(cAbs2(ref_index[0])); /* default value of dpl */
    ...
    if (dpl!=UNDEF)
      LogError(EC_ERROR,ONE,POSIT,"Extra information (all of dpl,grid,size) are given");
