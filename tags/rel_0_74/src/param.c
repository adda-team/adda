...
static const struct subopt_struct beam_opt[]={
  ...
  {"barton1","<width> <x> <y> <z>",
     "Gaussian beam by Barton, 1st order. The width and x, y, z coordinates \n"\
     "of the center of the beam must be specified (all in um).",4,B_BARTON1},
  {"barton3","<width> <x> <y> <z>",
     "Gaussian beam by Barton, 3rd order. The width and x, y, z coordinates \n"\
     "of the center of the beam must be specified (all in um).",4,B_BARTON3},
  {"barton5","<width> <x> <y> <z>",
     "Gaussian beam by Barton, 5th order. The width and x, y, z coordinates \n"\
     "of the center of the beam must be specified (all in um).",4,B_BARTON5},
  {"davis1","<width> <x> <y> <z>",
     "Gaussian beam by Davis, 1st order. The width and x, y, z coordinates \n"\
     "of the center of the beam must be specified (all in um).",4,B_DAVIS1},
  {"davis3","<width> <x> <y> <z>",
     "Gaussian beam by Davis, 3rd order. The width and x, y, z coordinates \n"\
     "of the center of the beam must be specified (all in um).",4,B_DAVIS3},
  {"lminus","<width> <x> <y> <z>",
     "Gaussian beam by 'L-' approximation. The width and x, y, z coordinates \n"\
     "of the center of the beam must be specified (all in um).",4,B_LMINUS},
  {NULL,NULL,NULL,0,0}
};
...
static void NargError(const int Narg,const char *exp)
...
{
  if (strlen(str)>=size)
    PrintErrorHelp("Too long argument to '-%s' option (only %d chars allowed).\n"\
...
PARSE_FUNC(beam)
{
  ...
  while (beam_opt[++i].name!=NULL) if (strcmp(argv[1],beam_opt[i].name)==0) {
    ...
    if (beamtype!=B_PLANE) {   /* should be improved (generalized) */
      ScanfError(argv[2],"%lf",&beam_w0);
      TestPositive(beam_w0,"beam width");
      ScanfError(argv[3],"%lf",&beam_x0);
      ScanfError(argv[4],"%lf",&beam_y0);
      ScanfError(argv[5],"%lf",&beam_z0);
    }
    /* stop search */
    found=TRUE;
    break;
  ...
  /* parses input parameters */
{
  ...
  for (i=1;i<argc;i++) {
    ...
    if(!found) PrintErrorHelp("Unknown option '-%s'",argv[i],exename,exeusage,exename);
    i+=Narg;
