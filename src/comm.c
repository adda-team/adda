...
void CatNFiles(const char *dir,const char *tmpl,const char *dest)
   /* cat several temporary files (one for each processor, names defines by the template 'temp'
      ...
      Employs system call to cat (not portable to windows), but the routine itself is used
      only in parallel mode */
{
  int buf_size,len,i;
  char *buf;

  buf_size=(MAX_TMP_FNAME)*nprocs+3*MAX_SYSTEM_CALL+strlen(dest)+strlen(dir);
  buf_size=MAX(buf_size,MAX_PARAGRAPH);  /* to be sure that error message will fit */
  if ((buf=(char *)malloc(buf_size*sizeof(char))) == NULL)
    LogError(EC_ERROR,ONE_POS,"Could not malloc buf");
  /* produce a command (cd into dir, then cat) */
  sprintf(buf,"cd \"%s\"; cat",dir);
  for (i=0;i<nprocs;i++) {
    len=strlen(buf);
    buf[len]=' ';               /* space */
    sprintf(buf+len+1,tmpl,i);  /* filename */
  }
  sprintf(buf+strlen(buf)," > %s",dest);
  /* execute command */
  if (system(buf)) {   /* check for system error */
    sprintf(buf,"Failed to combine files '%s ... %s' in directory '%%s' into file '%%s'."\
                "You can do it manually",tmpl,tmpl);
    LogError(EC_WARN,ONE_POS,buf,0,nprocs-1,dir,dest);
  }
  else for (i=0;i<nprocs;i++) {   /* remove temporary files */
    /* produce full path of tmp file */
    sprintf(buf,"%s/",directory);
    sprintf(buf+strlen(buf),tmpl,i);
    /* remove it */
    RemoveErr(buf,ONE_POS);
  }
  free(buf);
}
#endif
      ...
      works for all types; not optimized for long data (allocates memory at every call) */
{
  ...
  else LogError(EC_ERROR,ONE_POS,"MyBcast: variable type %d is not supported",type);
  /* allocate buffer */
      ...
      works for all types; not optimized for long data (allocates memory at every call) */
{
  ...
  else LogError(EC_ERROR,ONE_POS,"MyInnerProduct: variable type %d is not supported",type);
     ...
     /* Gather distributed arrays; works for all types */
{
  ...
  else LogError(EC_ERROR,ONE_POS,"AllGather: variable type %d is not supported",type);
