...
if "%intel_type%"=="classic" (
  set cc=icl
  set cflags=-Qstd=c99 -O1 -W5 -Wcheck -D_CRT_SECURE_NO_WARNINGS ^
             -Qdiag-disable:193,593,869,1011,1418,1419,1572,11074,11075,13000,13046
) else (
  set cc=icx
  set cflags=-Qstd=c99 -O1 -W4 -D_CRT_SECURE_NO_WARNINGS -Wno-return-type -Wno-unused-parameter -Wno-unused-function ^
...
if "%with_fortran%"=="true" (
  ...
  :: The following is required, since ifort on Windows makes function names in object files differently than on other OS
  :: Starting from Fortran 2003 it is recommended to use ISO_C_BINDING, but it is not a perfect solution for legacy code
  :: For now ADDA requires a single cross-language function call, so we replace the naming in C sources
  call set cflags=%%cflags%% -Dpropaespacelibreintadda_=PROPAESPACELIBREINTADDA
) else (
  call set cflags=%%cflags%% -DNO_FORTRAN 
)
cd seq
:: to avoid influence of previous compilations (since we do not have a proper Makefile)
del *.obj
%cc% -c -I"%fftw_path%" %cflags% %cfiles%
if "%with_fortran%"=="true" %fc% -c %fflags% %ffiles%
