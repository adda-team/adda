...
PARSE_FUNC(granul)
{
  ...
  ScanfError(argv[1],"%lf",&gr_vf);
  TestRange(gr_vf,"volume fraction",0,PI_OVER_SIX);
  ScanfError(argv[2],"%lf",&gr_d);
  TestPositive(gr_d,"diameter");
  if (Narg==3) {
    ScanfError(argv[3],"%i",&gr_mat);
  ...
  /* print info to stdout and logfile */
{
   ...
   if (ringid==ROOT) {
    ...
    if (Nmat>1) {
      fprintf(logfile,"  per domain: 1. %d\n",mat_count[0]);
      for (i=1;i<Nmat;i++) fprintf(logfile,"              %d. %d\n",i+1,mat_count[i]);
    }
