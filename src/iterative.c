...
double inprodR_0;   /* r_0; saved and load on checkpoint */
double epsB;        /* stopping criterion */
double resid_scale; /* scale to get square of relative error */
double prev_err;    /* previous Rel.Error; used in ProgressReport, initilized in IterativeSolver */
int method;         /* iteration method */
int count;          /* iteration count */
int counter;        /* number of successive iterations without residual decrease */
int max_count;      /* maximum allowed value of counter */
int chp_exit;       /* checkpoint occured - exit */
int chp_skip;       /* skip checkpoint, even if it is time to do */
...
iter_data_type iter_data;  /* actually the structure */
     ...
     on the same machine (number of processors) and with the same command line   */
{
  ...
  if (ringid==ROOT) {
    ...
    sprintf(fname,"%s/chp.log",chp_dir);
    if ((chp_file=fopen(fname,"w"))==NULL) {
      sprintf(sbuffer,"mkdir %s",chp_dir);
      system(sbuffer);
      if ((chp_file=fopen(fname,"w"))==NULL)
        LogError(EC_ERROR,ONE,POSIT,"Failed to open file 'chp.log'");
    }
  ...
  sprintf(fname,"%s/chp.%d",chp_dir,ringid);
  if ((chp_file=fopen(fname,"wb"))==NULL)
  ...
  if (fwrite(xvec,sizeof(doublecomplex),nlocalRows,chp_file)!=nlocalRows)
    LogError(EC_ERROR,ALL,POSIT,"Failed writing to file '%s'",fname);
  ...
  Synchronize();
  printz("Checkpoint (iteration) saved\n");
  fprintz(logfile,"Checkpoint (iteration) saved\n");
     ...
     on the same machine (number of processors) and with the same command line  */
{
  ...
  sprintf(fname,"%s/chp.%d",chp_dir,ringid);
  if ((chp_file=fopen(fname,"rb"))==NULL)
    LogError(EC_ERROR,ALL,POSIT,"Failed to open file '%s'",fname);
  /* check for consistency */
  fread(&method_new,sizeof(int),1,chp_file);
  if (method_new!=method)
    LogError(EC_ERROR,ALL,POSIT,"File '%s' is for different iterative method",fname);
  fread(&nlocalRows_new,sizeof(int),1,chp_file);
  if (nlocalRows_new!=nlocalRows)
    LogError(EC_ERROR,ALL,POSIT,"File '%s' is for different vector size",fname);
  ...
  if (fread(xvec,sizeof(doublecomplex),nlocalRows,chp_file)!=nlocalRows)
    LogError(EC_ERROR,ALL,POSIT,"Failed reading from file '%s'",fname);
  ...
  if(fread(&ch,1,1,chp_file)!=0) LogError(EC_ERROR,ALL,POSIT,"File '%s' is too long",fname);
  fclose(chp_file);
  /* initialize auxiliary variables */
  epsB=eps*eps*inprodR_0;
  if (ringid==ROOT) prev_err=sqrt(resid_scale*inprodR);
  /* print info */
  printz("Checkpoint (iteration) loaded\n");
  fprintz(logfile,"Checkpoint (iteration) loaded\n");
    ...
    /* Bi-Conjugate Gradient for Complex Symmetric systems */
{
  ...
  while (inprodR>=epsB && count<=maxiter && counter<=max_count && !chp_exit) {
    ...
    if (sqrt(cAbs2(ro_new))<EPS_BICG_CS) LogError(EC_ERROR,ONE,POSIT,
    ...
    if (sqrt(cAbs2(mu))<EPS_BICG_CS) LogError(EC_ERROR,ONE,POSIT,
        "BiCG_CS fails: zero ro_new (%.2g%+.2gi).",ro_new[RE],ro_new[IM]);
    /* alpha_k=ro_k/mu_k */
...
int IterativeSolver(int method_in)
       ...
       do common initialization part */
{
  ...
  /* error output */
  if (count>maxiter) LogError(EC_ERROR,ONE,POSIT,
    "Iterations haven't converged in maximum allowed number of iterations (%d)",maxiter);
  else if (counter>max_count) LogError(EC_ERROR,ONE,POSIT,
