...
int make_particle (double **coord,	/* coordinate vector */
	           int store,           /* store in coord table (true,false) */
	           int shape,           /* particle shape */
	           int jagged)          /* bigger dipoles used (jagged x jagged x jagged) - not really working yet */
     /* generate a particle; returns number of real (not void) dipoles, which is used for volume correction
        algoritm for counting real dipoles works only for basic shapes */
{
  ...
  if (shape == AGGREGATE) {
	...
	/*	if (x_<dthick/2.0 && x_>-dthick/2.0) { 
      ...
      /*      if  (shape==SDISK_ROT) {
	    ...
	    if (store==true) if (dipcount>=local_d0 && dipcount<local_d1) {
              position[3*index]=i;
              position[3*index+1]=j;
              position[3*index+2]=k+(boxZ+1)/2; /* for PARALLEL impl. */
      ...
      */
{
  ...
  fprintz(pbm,"# minimal amplitude:%.8g\n",min);
  fprintz(pbm,"# maximal amplitude:%.8g\n",max);
  fprintz(pbm,"%i %i\n",X,Y);
  fprintz(pbm,"255\n");
  ...
  for(i=0;i<nDip;i++) {
    ...
    if (position[3*i+0]==0) fprintz(fields,"%i %i %i %.8g %.8g %.8g %.8g %.8g %.8g\n",
