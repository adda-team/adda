...
int gr_N;                        /* number of granules */
double gr_vf_real;               /* actual granules volume fraction */
int mat_count[MAX_NMAT+1];       /* number of dipoles in each domain */
...
#define MAX_GR_SET 0xFFFF    /* determined by maximum size of unsigned short */
...
{
  int i,j,k,n,nd,boxXY,count,count_gr,zerofit,false_count,last;
  int index,index1,index2;             /* indices for dipole grid */
  int dom_index,dom_index1,dom_index2; /* indices for auxilliary grid */
  int gX,gY,gZ,gXY,gr_gN;              /* auxilliary grid dimensions and their products */
  int avail;                           /* number of available (free) domain cells */
  ...
  int *vfit;             /* results of granule fitting on the grid */
  ...
  max_Ngr=(((int)ceil((x1-x0)/Di))+1)*(((int)ceil((y1-y0)/Di))+1)*(((int)ceil((z1-z0)/Di))+1);
  if (max_Ngr>MAX_GR_SET) max_Ngr=MAX_GR_SET;
  ...
  MALLOC_VECTOR(vfit,int,max_Ngr,ALL);
  if (!sm_gr && locgZ!=0) {
  ...
  n=nd=count=count_gr=false_count=0;
  ...
  while (n<gr_N) {
    if (sm_gr) {  /* small granules */
      ...
      if (ringid==ROOT) {
        cur_Ngr=MIN((int)ceil((gr_N-n)*overhead),max_Ngr);
        /* generate points and quick check */
        ig=false_count=0;
        for (i=0;i<gr_gN;i++) occup[i]=MAX_GR_SET;  /* used as undef */
        while (ig<cur_Ngr) {
          count++;
          false_count++;
          fits=TRUE;
      ...
      if (ringid==ROOT) {
        ...
        avail=0;
        for (i=0;i<gr_gN;i++) if (dom[i]!=0xFF) avail++;
        cur_Ngr=MIN(avail,max_Ngr);
    ...
    if (Nfit!=0) zerofit=0;
      ...
      if (zerofit>MAX_ZERO_FITS) {
        MyInnerProduct(&nd,int_type,1,&Timing_Granul_comm);
        LogError(EC_ERROR,ONE_POS,
          "The granule generator failed to reach required volume fraction (%g) of granules. "\
          "%d granules were successfully placed up to a volume fraction of %g",
          gr_vf,n,nd/(double)(mat_count[gr_mat]));
      }
    }
  }
  PRINTZ("Granule generator: total random placements= %d (efficiency 1 = %g)\n"\
         "                   possible granules= %d (efficiency 2 = %g)\n",
  ...
  /* creates a particle; initializes all dipoles counts, dpl, gridspace */
{
  ...
  MALLOC_VECTOR(DipoleCoord_tmp,double,3*local_Ndip,ALL);
  MALLOC_VECTOR(position_tmp,ushort,3*local_Ndip,ALL);
...
/*        else if  (shape==SH_SDISK_ROT) {
  ...
  MyInnerProduct(mat_count,int_type,Nmat+1,NULL);
  ...
  if (sh_granul) {
    ...
    gr_N=(int)ceil(tmp2*tmp1*tmp1*tmp1);
    ...
    gr_vf_real=mat_count[Nmat-1]/(double)(mat_count[gr_mat]);
