	      ...
	      int her)              /* 0 for non-hermitic, 1 for hermetic */
{
  ...
  clock_t tstart,tprof[15],Timing_MatVecMult,Timing_MatVecFFT,Timing_MatVecAlloc;
  static mvcounter=0;
  int start;
  ...
  tprof[1]=clock();
  Timing_MatVecAlloc=tprof[1]-tprof[0];    /* timing */
  ...
  for(Xcomp=0;Xcomp<3;Xcomp++) { 
    ...
    tprof[3]=clock();                           /* timing */
    Timing_MatVecMult+=tprof[3]-tprof[2];
    ...
    tprof[4]=clock();
    Timing_MatVecFFT+=tprof[4]-tprof[3];         /* timing */
  ...
  for(sliceX=local_x0;sliceX<local_x1;sliceX++) {
...
#endif
    profile[1]+=clock();
    tprof[6]=clock();                         /* timing */
    Timing_MatVecMult+=tprof[6]-tprof[5];
    ...
    tprof[7]=clock();                          /* timing */
    Timing_MatVecFFT+=tprof[7]-tprof[6];
...
/*#ifndef PARALLEL
    ...
    tprof[8]=clock();
    Timing_MatVecMult+=tprof[8]-tprof[7];       /* timing */
    profile[2]+=clock();
    ...
    tprof[9]=clock();                          /* timing */
    Timing_MatVecFFT+=tprof[9]-tprof[8];
    ...
    tprof[10]=clock();
    Timing_MatVecMult+=tprof[10]-tprof[9];       /* timing */
  ...
  for(Xcomp=0;Xcomp<3;Xcomp++) { 
    ...
    tprof[12]=clock();                          /* timing */
    Timing_MatVecFFT+=tprof[12]-tprof[11];
	...
	*inprod+=resultvec[j+Xcomp].r*resultvec[j+Xcomp].r + resultvec[j+Xcomp].i*resultvec[j+Xcomp].i;
    ...
    tprof[13]=clock();
    Timing_MatVecMult+=tprof[13]-tprof[12];       /* timing */
  ...
  tprof[14]=clock();
  Timing_MatVecAlloc+=tprof[14]-tprof[13];    /* timing */
  ...
  /* temporarry code */
/*  printz("Allocation  = %4.4f\n"\
         "Arithmetics = %4.4f\n"\
         "FFT         = %4.4f\n",
         TO_SEC(Timing_MatVecAlloc),TO_SEC(Timing_MatVecMult),TO_SEC(Timing_MatVecFFT));
  stop(1); */
}
