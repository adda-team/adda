...
static inline void InterParams(double qvec[static 3],double qmunu[static 6],double *rr,double *invr3,doublecomplex *kr,
		doublecomplex *kr2)
...
// Interaction term between two dipoles with integration of Green's tensor. See InterTerm_poi for more details.
{
	...
	if (igt_lim==UNDEF || DotProd(qvec,qvec)<=igtLimR2 ) {
		 ...
		 */
		propaespacelibreintadda_(qvec,&RealWaveNum,&dsX,&dsY,&dsZ,&igt_eps,tmp,&ifail);
		if (ifail!=0) {
