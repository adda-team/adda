...
{
  if (warn_buf[0]!=0) {
    if (ringid==0) fprintf(logfile,warn_buf);
    else if ((logfile=fopen(logname,"a"))!=NULL) {
      fprintf(logfile,warn_buf);
      fclose(logfile);
    }
    /* empty buffer */
    warn_buf[0]=0;
...
void MkDirErr(const char *dir,const int who,const char *err_fname,const int lineN)
   /* make directory and check for error.
      Though most systems have C routines for mkdir, they are located in different
      header files. The most portable way to call them seems through  */
{
