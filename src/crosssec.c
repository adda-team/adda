   ...
   /* scan integration parameters for angles from file */
{
  ...
  ReadLineStart(file,fname,buf,buf_size,"Jmin=");
  if (sscanf(buf,"Jmin=%d",&(b->Jmin))!=1) return 1;
  ...
  ReadLineStart(file,fname,buf,buf_size,"periodic=");
  if (sscanf(buf,"periodic=%s",temp)!=1) return 1;
  ...
  if ((a->val=(double *) malloc(a->N*sizeof(double)))==NULL)
    LogError(EC_ERROR,ALL_POS,"Could not malloc integration array");
   ...
   /* scan range or set of angles (theta or phi) from file (used for scat_grid) */
{
  ...
  if ((a->val=(double *) malloc(a->N*sizeof(double)))==NULL)
    LogError(EC_ERROR,ALL_POS,"Could not malloc angle array");
  memory += a->N*sizeof(double);
