...
void CheckOverflow(const double size,OTHER_ARGUMENTS)
  /* checks if size can fit into size_t type,
     otherwise overflow will happen before memory allocation */
{
...
double *doubleVector2(const size_t nl,const size_t nh,OTHER_ARGUMENTS)
  /* allocates double vector with indices from nl to nh; all arguments must be non-negative;
     and nh>=nl */
{
  double *v;
  size_t size;
