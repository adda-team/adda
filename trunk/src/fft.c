 ...
 */
{
	 ...
	 * tables, etc (e.g. as is done in GSL library). But currently this do not seem to consume a
	 * significant portion of the total simulation time.
	 */
		...
		if (kr*rn < G_BOUND_CLOSE) {
			...
			if (kr < G_BOUND_MEDIAN) {
				...
				if (!inter_avg) {
					...
					temp = 3*qa*(dmunu-7*qmunu) + 6*dmunu*qvec[mu]*prop[mu]
					     - 7*(dmunu-9*qmunu)*DotProd(qavec,q2)
