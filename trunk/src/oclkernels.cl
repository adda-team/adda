...
{
return ( a[0].s0*a[0].s0 + a[0].s1*a[0].s1
                           + a[1].s0*a[1].s0 + a[1].s1*a[1].s1
...
__kernel void arith1(__global unsigned char *material,
                     __global unsigned short *position, 
                     __global double2 *cc_sqrt, //15x3
                     __global double2 *argvec, 
                     __global double2 *Xmatrix, 
                     long local_Nsmall, 
                     long smallY, 
                     long gridX)
{
...
__kernel void arith2(
                    __global double2 *Xmatrix,
                    __global double2 *slices,
                    long gridZ,
                    long smallY,
                    long gridX,
                    long gridYZ,
                    ...
                    )
{
    ...
    for (xcomp=0;xcomp<3;xcomp++)
          slices[i+xcomp*gridYZ]=Xmatrix[j+xcomp*local_Nsmall];
}
...
__kernel void arith3(
        __global double2 *slices_tr,
        __global double2 *Dmatrix,
        long local_x0,
        long smallY,
        long smallZ,
        long gridX,
    ...
    ) 
{
    ...
    for (Xcomp=0;Xcomp<3;Xcomp++)
        xv[Xcomp]=slices_tr[i+Xcomp*gridY*gridZ]; //works, because of the double2 vector type
    size_t j;
    ...
    for (int f=0;f<6;f++) //maybe could be done more efficient TODO
        fmat[f]=Dmatrix[j+f];
    ...
    for (Xcomp=0;Xcomp<3;Xcomp++)
    {
        slices_tr[i+Xcomp*gridY*gridZ]=yv[Xcomp];
    }
}
...
{   
  ...
  //  barrier(CLK_LOCAL_MEM_FENCE);
    for (xcomp=0;xcomp<3;xcomp++)
            {
          Xmatrix[j+xcomp*local_Nsmall]=slices[i+xcomp*gridYZ];
          }
}
...
__kernel void arith5(
        __global unsigned char *material,
        __global unsigned short *position, 
        __global double2 *cc_sqrt,
        __global double2 *argvec,
        __global double2 *Xmatrix,
        long local_Nsmall,
        long smallY,
...
//    size_t k = get_global_id(2);
    ...
    for (int k=0;k<3;k++)
        output[idy*height+idz+k*wth]=input[idz*width+idy+k*wth];
}
