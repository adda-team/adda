...
void main (int argc, char *argv[])
{
  ...
  i=1; if (argc>1) do {
	...
	sscanf(argv[++i],"%lf",&gam_deg);
      ...
      if (Narg!=1) {
        printz("Illegal number of arguments (%d) to -jagged option\n",Narg);
        stop(1);
      }
      sscanf(argv[++i],"%d",&jagged);
    }
    else if (strcmp(argv[i],"-novolcor")==0)
      volcor=false;
    else {
      ...
      printz("    [-Csca] [-vec] [-asym] [-Cpr_mat] [-store_force] [-busy] [-store] [-jagged %%d]\n");
  ...
  if (dpl==UNDEF) {
    if (volcor && volume_ratio!=UNDEF) {                      /* dpl is corrected to give correct volume */
      realdips=make_particle(NULL,false,shape,jagged);
      dpl=LAMBDA*pow(realdips/volume_ratio,.333333333333)/sizeX;
    }
  ...
  printz("lambda:%.8g      m:%.8g%+.8gi     Dipoles/lambda:%.8g\n",LAMBDA,ref_index[0].r,ref_index[0].i,dpl);
  printz("epsilon square:%.8g\n",eps0);
  ...
  if (alph_deg!=0 || bet_deg!=0 || gam_deg!=0) {
    ...
    fprintz(logfile,"New incident propagation vector:(%.8g,%.8g,%.8g)\n",prop[0],prop[1],prop[2]);
    fprintz(logfile,"New incident polarization Y(par):(%.8g,%.8g,%.8g)\n",incPolY[0],incPolY[1],incPolY[2]);
    fprintz(logfile,"New incident polarization X(per):(%.8g,%.8g,%.8g)\n\n",incPolX[0],incPolX[1],incPolX[2]);
  }

  fprintz(logfile,"Dipoles/lambda:%.8g\n",dpl);  
  if (realdips!=UNDEF) fprintz(logfile,"      (Volume correction used)\n");
  fprintz(logfile,"epsilon square:%.8g\n",eps0);
